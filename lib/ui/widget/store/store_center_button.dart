...
class StoreCenterButton extends StatelessWidget {
  ...
  StoreCenterButton({this.storeModel});

  @override
  Widget build(BuildContext context) {
    double w = MediaQuery.of(context).size.width/2 - wPadding;
    bool isCoupon = storeModel.summary.couponType != 0;
                      ...
                      children: <Widget>[
                        Text(
                            isAlreadyIssueCoupon
                                ? '${storeModel.summary.couponValue}원 쿠폰 받기 완료'
                                : '${storeModel.summary.couponValue}원 쿠폰 받기',
  ...
  void _onPressedCouponButton() {
    if(storeModel.summary.couponType == 0) return;
    print('coupon button!!');
  }
